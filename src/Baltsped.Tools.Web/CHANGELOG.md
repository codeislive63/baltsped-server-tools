...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest baltsped.tools.web -- --template=react-ts  --no-rolldown --no-immediate`.
- Обновление `vite.config.ts` с портом.
- Создание файла проекта (`baltsped.tools.web.esproj`).
